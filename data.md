...
| **TfL QSI & Mileage per-route PDFs** (`bus.data.tfl.gov.uk/boroughreports/routes/performance-route-{id}.pdf`) | None | Per-route, per-4-week-period **Mileage Operated (%)** and reliability (EWT / on-time) — the authoritative "actually operated vs scheduled" record, 26 rolling periods | One PDF per route (~675). Cached on `Last-Modified`; TfL republishes quarterly. Parsed with `pdfjs-dist`. |
| **DfT Street Manager open data** (SNS push → `/api/streetworks` Pages Function → `streetworks-inbox` branch → drained to the archive) | Free registration (org name + contact + THIS endpoint URL at manage-roadworks.service.gov.uk/open-data-onboarding) | The statutory record of EVERY permitted street work in England, filtered in-function to the 33 London boroughs + TfL via the `ha_org` message attribute (in production a bare GeoPlace SWA org code — 5030…5990 + 20 for TfL — mapped to names in-function; verified against GeoPlace's SWA_ORG list) | Push-only upstream (no pull API for open-data users). Signature-verified (SigV1/V2, cert host pinned to sns.eu-west-2), topics pinned to DfT's prod-permit/activity/section-58 ARNs. Storage is git itself: the function commits one file per event to the `streetworks-inbox` branch via the GitHub contents API (unique filenames — concurrent head races 409 and SNS redelivers), and the status workflow folds them into `data/api/streetworks-history.json`, then resets the branch to a single parentless commit via `--force-with-lease` (an event landing mid-drain fails the lease and is retried — nothing lost, no unbounded history at ~1000s of events/day). Live since 2026-09-18; payload coordinates are WKT in EPSG:27700, converted to WGS84 [lat, lng] at drain time (Airy 1830 inverse TM + Helmert, ~5 m) so the route-map street-works layer can draw them. Setup: fine-grained GitHub PAT (this repo, Contents read/write) as Pages env var `GITHUB_TOKEN`; exclude `streetworks-inbox` from Pages PREVIEW builds; GET `/api/streetworks` until `ready:true`; then register. Drain soft-skips until the branch exists. |
| **TfL Road disruptions (TIMS)** (`api.tfl.gov.uk/Road/all/Disruption`) | None | Live roadworks & road disruptions from TfL's 24/7 traffic control centre (~120-130 live; mostly utility/TfL/borough Works), corridor-joined to bus routes within 30 m and accumulated into `data/source/roadworks-history.json` per status refresh | Live-only upstream, like the diversion feed — the accumulation IS the history. Coverage = TfL Road Network + whatever TIMS is managing, not the statutory record of every borough street work: that is **DfT Street Manager open data** (free registration at gov.uk/guidance/find-and-use-roadworks-data, credentialed pull API) — the documented upgrade path if borough-level completeness is needed. Commercial aggregator (one.network) not required. |
| **Diversion snapshots (self-accumulated)** | — | **Only source of diversion HISTORY**: TfL's Unified API serves current disruptions only, and no public archive exists — so every status refresh's `route-diversions.json` snapshot (~5-8/day) is merged into a rolling archive (`data/source/diversion-history.json`) keyed by (route, category, reason text) | Category `PlannedWork`/`Event` = scheduled, `RealTime` = unscheduled. A reason-text edit by TfL starts a new entry (deliberate — the edit is part of the history). Archive starts 2026-09-06 (oldest committed snapshot). |
| **london-bus-routes.fandom.com Tender Results pages** (MediaWiki API) | None | **Supplementary** provisional tender awards — TfL's results register lags the weekly award bulletins by months; the community wiki tracks them same-week (cross-checked: 100% agreement on winner, ~99% on date/accepted bid across 163 overlapping rows) | Served flagged `provisional`; only winner/date/current operator/PVR/vehicles/accepted bid carried (lowest/highest/£-per-mile are where wiki typos live). A provisional row drops out automatically once TfL publishes the real award page. |
...
| `propulsion` | `hybrid` \| `electric` \| `hydrogen` \| `diesel` \| null | **DVLA fleet aggregator** (mode of `fuelType` across observed regs) | `vehicle-lookup.json` → LBR-string heuristic → last-known-good | DVLA wins because it's the authoritative fuel-type register. The LBR-string heuristic stays as fallback for routes the arrivals snapshot didn't cover (e.g. routes with no buses running at the snapshot moment). A stale LBR non-diesel claim is also overridden on unambiguous DVLA evidence (core fleet ≥5 regs): LBR `hybrid` + DVLA-dominant `electric` → `electric` (the route converted), and LBR `electric` with zero ELECTRICITY regs in the core fleet → DVLA-dominant (the route is not electric). Hydrogen claims are never overridden. |
| `make` | string \| null | DVLA fleet aggregator (day-weighted mode of `make` across the **core fleet** — each reg weighted by distinct days seen, so a long-serving allocation outvotes short-lived cover blocks) | Last-known-good | DVLA's manufacturer (e.g. `VOLVO`, `ALEXANDER DENNIS`). Body builder isn't separately stored — `vehicleType` carries the chassis+body string from LBR. The core-fleet filter (see §3 step 13) drops one-off cover/reserve buses so an emergency vehicle that ran the route once can't define its make. |
...
- **`data/source/diversion-history.json`** — rolling diversion archive (see Sources). Grown by `build-diversion-history.js` after every status refresh; `--seed` replays all committed snapshots from git. Served as `data/api/diversion-history.json` (entries sorted by lastSeen desc, with per-entry observed-day lists, `scheduled` and `active` flags).
- **`data/api/scheduled-mileage.json`** — per-route daily SCHEDULED km estimate: TfL timetable journeys per day-type × per-direction geometry length (longest run). Estimate, clearly labelled: short workings counted at full route length. The operated side deliberately stays TfL's per-period figure — daily operated km cannot be honestly produced without continuous tracking.
- **`data/source/route-performance.json` / `data/api/route-performance.json`** — per-route per-period Mileage Operated % + reliability from TfL's QSI PDFs, with per-route `Last-Modified` cache.
...
| Freshness | Every cache file's `generatedAt` is within 8 days for weekly-refreshed files, 14 days for cached files updated only when upstream moves (programme). |
...
| `_lib/cache.js` | `loadJsonCache`, `atomicWriteJson`, `installSignalFlush` | sticky-cache fetchers (vehicle-fleet, tenders) |
| `_lib/pdf.js` | `extractPdfRowsByPage`, `extractPdfRows`, `initPdfWorker` | PDF fetcher (tender-programme) |
| `_lib/sanitize.js` | `sanitizeText`, `sanitizeRecord` | every fetcher + every build step at the JSON-write boundary |

`userAgent()` reads the package version at module load so every outbound HTTP request stamps with the same value (`london-buses-map/2.8 (tenders)`) — no per-script version drift.
