...
**Cadence:** weekly, Mondays 09:00 UTC, via GitHub Actions (`.github/workflows/refresh-data.yml`). Timed for AM peak so TfL `/Line/<id>/Arrivals` returns a representative active fleet. A separate twice-weekly heartbeat workflow (`.github/workflows/supabase-heartbeat.yml`, Thu+Sun 12:00 UTC) issues a single PostgREST read against `route_snapshots` to keep the free-tier Supabase project out of its 7-day inactivity auto-pause — it would otherwise sit on the edge of the cadence and pause whenever Monday's run slipped.
**Orchestrator:** `scripts/refresh.js` — runs the 17 pipeline steps in sequence; network-bound fetch steps soft-fail so one flaky source doesn't abort the week. Pure build steps hard-fail. Step 16 (`audit-data.js`) is a hard-fail data-quality gate so broken data can't be committed.
...
- **Garage capacity is manual** — per-garage operating-centre vehicle capacity (DVSA operator licence) is **not available via any bulk/API source**. The data.gov.uk OLBS CSV (`OLBSLicenceReport_*`) carries only the *licence-wide* authorised total (e.g. London General's one licence = 2507 across 19 garages), not per-operating-centre. The web service that shows the per-OC figure (`vehicle-operator-licensing.service.gov.uk/view-details/licence/<id>`) is **Incapsula bot-protected — HTTP 403 to fetch, the search endpoint, AND headless Puppeteer**. So `data/garage-capacity.json` is hand-filled from the licence pages (one page lists all of an operator's garages; match to a garage by postcode). `build-garage-locations.js` joins it into `garage-locations.json` by garage code. Capacity changes only at licence variation (~yearly), so manual upkeep is light. Frontend shows it as a Capacity + Free-space (`1 − PVR/capacity`) KPI in the garage and operator drawers; operator-level only renders when *every* garage of that operator has a capacity on file.
...
- `data/garage-capacity.json` — per-garage operating-centre vehicle capacity (DVSA operator licence), keyed by TfL garage code. Hand-filled from the licence pages because the figure isn't in any bulk/API export (see "Garage capacity is manual" in §6). Joined into `garage-locations.json` by `build-garage-locations.js`; run `npm run build-garage-locations` after editing.
