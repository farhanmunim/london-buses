...
- **`data/garage-locations.json`** — geocoded garage records (code, name, operator, lat/lon, PVR, capacity). Consumed by the frontend for garage map markers **and** the sidebar Garage filter (a dropdown of garages grouped by operator that intersects the route set, like the bus-stop filter). Each garage's route set is derived client-side from `route_classifications.json`'s `garageCode` in `ui.js`.
