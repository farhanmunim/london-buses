...
**Cadence:** GitHub Actions is the scheduler (this repo is the whole data platform — Actions = compute, git = database, Cloudflare Pages serves `data/api/*.json` as the API). Four workflows share one `data-refresh` concurrency group: **nightly full refresh** 03:17 UTC (`weekly-refresh.yml` — historical filename; renaming would orphan its run history) — the night-hours slot also gives the arrivals sweep night-network coverage; **status intraday** 6×/day at :41 (`refresh-status.yml` — line status + diversion register only); **tenders** 12:20 + 15:20 UTC (`refresh-tenders.yml` — TfL publishes awards ~1pm/~3pm UK; the second run lands after both windows in winter too); **fleet sweeps** 11:20 + 23:20 UTC (`refresh-fleet.yml` — incremental arrivals samples replacing the retired 24/7 monitoring). All commit only when content changed, so quiet runs cost no Cloudflare Pages build (~11 builds/day worst case, within the 500/month free tier).
...
**Frontend data sources:** the browser data layer (`js/api.js`) is **API-first against the Atlas public API** (`https://atlas.farhan.app/api/v1` — same author, same upstream warehouse; CORS-open, no key) for the datasets it serves: garage locations (`/garages`, merged join-safely with `garage-locations.json` — the API refreshes name/position/PVR/capacity per code, the committed file keeps street addresses, the operator vocabulary `route_classifications.json` joins on, and garages the API doesn't list) tender awards (`/tenders`, adapted to the `source/tenders.json` shape with ISO dates and `reason_not_lowest` overlaid from the snapshot), and the per-route reliability block (`/route-performance` — `serviceClass`, EWT/OTP actuals + MPS — overlaid onto `route_classifications.json` at load; both sides parse the same TfL QSI publications, but the API re-checks daily, so the overlay is never staler than the committed build). Three **API-only** datasets (no bundled equivalent) power UI that hides when the API is unreachable: `line-status` (live per-route status chip on the route card + the Overview network line), `crowding` (TfL BUSTO — the route card's Crowding section), and `manifest` (per-dataset freshness behind the footer's Refreshed/Next dates; Atlas's warehouse pipeline runs daily ~03:17 UTC). Each fallback-backed API path falls back to the committed files automatically, so everything in this document remains the production read layer of record. All other frontend datasets (per-route geometry, classifications, stops, destinations, overview) load only from the committed files: the API doesn't yet serve them at the fidelity the UI needs (full-res per-direction geometry, the ~71 geometry-only route variants TfL itself doesn't list, stop `indicator`/`towards`, paint-mode properties, destination labels, and the multi-source classification enrichment — frequency, deck, fleet observation, tender derivations).
...
| 9 | `fetch-tenders.js` | `tfl.gov.uk/forms/13923.aspx` (discovery) → `tfl.gov.uk/forms/13796.aspx?btID=…` (per award) | `source/tenders.json` (sticky cache, force-committed) | Soft fail. Awards are immutable; only new btIDs are fetched. Per-run cap of 4,000; flush every 100 + SIGTERM handler. |
...
| 15 | `build-api.js` | every committed dataset above + `garage-overrides.json` + previous `data/api/*.json` | `data/api/*.json` — the served faux-API: `routes` (bare array, active TfL routes), `route-meta`, `garages` (source ∪ previous build, so licence-sweep garages survive), `fleet` (14-day recency window on sightings; per-route propulsion counts), `vehicles` (byReg), `tenders` (byId + byRoute with lowest/highest/accepted bids), `routes-overview` (active-route filter, v2 property names), `manifest` | Hard fail (pure transformation). Sticky per-field last-known-good merge: a thin upstream run never blanks a served field. |
| 16 | `audit-data.js` | all build outputs | `data/audit/data-quality.json` | **Hard fail on CRITICAL.** Walks every record in `route_classifications`, `vehicle-fleet`, `garages.geojson`, and `route-vehicles`. Three severities: CRITICAL (schema violations — broken counts, malformed dates, unknown enums, duplicate codes, missing geometry — hard-fails refresh), WARN (out-of-range values, sparse fleet coverage, cross-source disagreements incl. `make↔model`, freshness >8 days — logged but non-fatal), INFO (coverage statistics). 13-route "ghost" allowlist (TfL-internal geometry that returns HTTP 404 from `/Line/{id}`) lives at the top of the script; routes outside that set with null operator/garage trigger a WARN. Pass `--strict` to fail on WARN too. |
