...
**Frontend data sources:** both front-ends read this repo's own faux-API — `data/api/*.json`, assembled by `scripts/build-api.js` from the committed primary datasets and served as static JSON by Cloudflare Pages (same response shapes the retired Atlas API served, so the apps and their test suites carried over by a URL change). v1's `js/api.js` overlays `/route-meta` and `/fleet` onto the bundled `route_classifications.json` join-safely and falls back to the committed files when a fetch fails; v2 reads the faux-API directly. Live layers are never stored: arrivals and per-route status come straight from TfL's CORS-open Unified API in the browser, live bus positions from the `/api/live/vehicles` Pages Function (BODS SIRI-VM). Per-route geometry, stops, destinations and the classification base still load from their committed files.
...
| 9 | `fetch-tenders.js` | `tfl.gov.uk/forms/13923.aspx` (discovery) → `tfl.gov.uk/forms/13796.aspx?btID=…` (per award) | `source/tenders.json` (sticky cache, force-committed) | Soft fail. Awards are immutable; only new btIDs are fetched. Per-run cap of 4,000; flush every 100 + SIGTERM handler. TfL's forms WAF intermittently 403s datacenter IPs (first seen 2026-08-28): fetches retry ×3 with jittered backoff and browser-ish Accept headers, and a still-blocked discovery **skips gracefully** when the cache is warm (exit 0, nothing rewritten) — `generatedAt` then only advances on successful sweeps, and the audit WARNs when it exceeds 7 days, so a persistent block surfaces without red workflow emails twice a day. |
