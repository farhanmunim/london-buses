...
**Cadence:** weekly, Mondays 09:00 UTC, via GitHub Actions (`.github/workflows/weekly-refresh.yml`). Timed for AM peak so TfL `/Line/<id>/Arrivals` returns a representative active fleet. (The old daily Supabase sampler and heartbeat workflows are gone — historical storage moved to the Atlas warehouse; see the Historical store section.)
