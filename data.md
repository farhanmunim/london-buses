...
- **`data/source/route-vehicles.json`** — `{ routes: { [routeId]: [{ reg, firstSeenAt, lastSeenAt, sightings, days }, …] } }`. Per-route registrations observed in TfL arrivals over the trailing 56 days. `sightings` = number of runs the reg appeared on the route; `days` = distinct calendar dates seen. These quantify recurrence so the fleet aggregator can separate a route's core fleet from one-off cover/reserve buses (legacy entries that predate the counts are back-filled as a single sighting on load). Force-committed; coverage grows week-over-week.
...
| 8 | `fetch-route-vehicles.js` | TfL `/Line/<id>/Arrivals` (Monday peak snapshot) | `source/route-vehicles.json` (registrations per route with `sightings`/`days` recurrence counts, accumulated week-over-week with 56-day TTL) | Soft fail. Each Monday snapshot only sees buses currently running; the rolling-TTL accumulator builds full per-route fleet coverage over a few weeks, and `sightings`/`days` accrue so the build step can tell regulars from one-offs. |
