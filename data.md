...
| **DfT Street Manager open data** (SNS push → `/api/streetworks` Pages Function → `streetworks-inbox` branch → drained to the archive) | Free registration (org name + contact + THIS endpoint URL at manage-roadworks.service.gov.uk/open-data-onboarding) | The statutory record of EVERY permitted street work in England, filtered in-function to the 33 London boroughs + TfL via the `ha_org` message attribute (in production a bare GeoPlace SWA org code — 5030…5990 + 20 for TfL — mapped to names in-function; verified against GeoPlace's SWA_ORG list) | Push-only upstream (no pull API for open-data users). Signature-verified (SigV1/V2, cert host pinned to sns.eu-west-2), topics pinned to DfT's prod-permit/activity/section-58 ARNs. Storage is git itself: the function commits one file per event to the `streetworks-inbox` branch via the GitHub contents API (unique filenames — concurrent head races 409 and SNS redelivers), and the status workflow folds them into `data/api/streetworks-history.json`, then resets the branch to a single parentless commit via `--force-with-lease` (an event landing mid-drain fails the lease and is retried — nothing lost, no unbounded history at ~1000s of events/day). Live since 2026-09-18; payload coordinates are WKT in EPSG:27700. Setup: fine-grained GitHub PAT (this repo, Contents read/write) as Pages env var `GITHUB_TOKEN`; exclude `streetworks-inbox` from Pages PREVIEW builds; GET `/api/streetworks` until `ready:true`; then register. Drain soft-skips until the branch exists. |
