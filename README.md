...
See [agent.md](agent.md) for the full architecture, data pipeline, and repository structure.
...
- **Route performance PDF** — `HEAD` first, compare `Last-Modified` against cached value; skip read if unchanged (PDF only republishes every ~4 weeks)
- **Per-route MPS PDFs** — per-route `HEAD` with 28-day TTL; cold-start week reads ~700 small PDFs, steady-state weeks are mostly HEAD-only and finish in seconds
