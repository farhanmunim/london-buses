...
The frontend data layer (`js/api.js`) is API-first against the [Atlas public API](https://atlas.farhan.app/api/v1) for every dataset it serves at UI fidelity — garage locations (merged join-safely with the committed record), tender-award history, per-route reliability (EWT/OTP + MPS), route meta (garage/PVR, with gap-fill for operator, vehicle model and propulsion) and fleet stats (all overlaid onto the classifications), live line status, crowding, and the freshness manifest — and falls back to the committed files automatically when the API is unreachable (API-only datasets simply hide their UI). The remainder (per-route geometry, the classification base, stops, destinations, the overview layer) loads from the committed weekly-refresh data; the `js/api.js` header lists the exact fidelity gaps that keep each one bundled until Atlas closes them. Historical storage lives in the Atlas warehouse (`/api/v1/history`) — this project no longer writes to any database.
