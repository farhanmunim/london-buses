...
- **DATA** Tender award checks also **chain off every status and fleet refresh** (`workflow_run` is event-driven, so it fires even when GitHub's congested cron drops the hourly slots), and the challenge-clearing browser runs headful under a virtual display so Cloudflare doesn't flag it as automation.
...
- **FIX** **Negative and phantom contract terms are gone.** Two honest-join guards in the pipeline: the newest award on a route only inherits route-meta's projected contract end when that contract is demonstrably its own (start dates within 60 days) — previously a future-dated win inherited the *outgoing* contract's end (negative/zero terms) and an old never-re-tendered award inherited a much newer contract's end (11–12-year terms); and an observed end within 90 days of the start (the same handover seen through two programme-PDF revisions, or a superseded re-award) no longer counts as a term. Every remaining term is 2–9 years, dominated by the real 5- and 7-year pattern.
- **UX** The page is renamed **Tender** (nav, heading and `#/tender` slug — the old `#/tenders` links redirect), and it is now **tabbed** — Awards and Programme sit behind two pills with entry counts, so the programme is discoverable without scrolling past 2,500 awards; `#/tenders/programme` deep-links the second tab (route pages' upcoming-tender flags land there).
...
- **DATA** Every stored CPI index value (199 months, Jan 2010 – Jul 2026) verified **exactly** against ONS's official Consumer Price Inflation detailed reference tables (Table 15a, 19 Aug 2026 edition) — no gaps, no differences. Derived YoY agrees with ONS's published 12-month rate within 0.1 pp everywhere; the residual is ONS computing its headline from unrounded indices while the CPA convention (and this page) uses the published 1-dp index — now noted under the table.
- **UX** The duplicate "Next ONS release" KPI tile is gone — the calendar chip and source line carry the date.
- **UX** The **next ONS release date** now headlines the CPI-CPA page as a calendar chip under the intro (it was already a KPI tile), and closes the source line under the table — three sightings, no missing it.
- **NEW** **CSV exports**: the CPI-CPA page exports its full series (`cpi-cpa.csv` — month, index, YoY, P2P, RA at full 8-dp precision) and links to the ONS D7BT series page; the network map's footer gains **Export CSV**, which downloads **every route's full record** (route, type, operator, company, garage, PVR/TVR, vehicle model, propulsion, length, contract dates) — deliberately ignoring any active filters. Exported cells are formula-injection-safe.
...
- **UX** The operating garage's accent ring is retired — the permanent "Operating from here" pill identifies it now, so the pin matches every other garage's design and the map loses one competing visual signal.
...
- **UX** Incidents render as a **severity-weighted density heatmap** (soft amber glow → deep red where collisions cluster; slight collisions weigh less than serious, serious less than fatal) with only **serious and fatal collisions kept as individual clickable dots** for the full STATS19 record — density at a glance, gravity on demand. Low bridges are **colour-graded by clearance**: red below double-deck height (< 4.2 m), amber tight (4.2–4.9 m), calm slate where a double-decker clears (≥ 4.9 m), with the height bands as a legend under the map.
- **UX** The map's layer buttons are now compact icon-only chips (hover/long-press for the name) — five layers no longer crowd the small map. Bridge markers slim down to small amber diamonds and collision dots stay minimal; **clicking any marker opens a detail card** — bridges show height (metric + signed imperial), structure type, road and borough; collisions show casualties, vehicles, road type, speed limit, junction, light, weather and surface.
