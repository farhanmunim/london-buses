...
- **UX** Live buses are one colour now — solid blue with a white ring, whatever the direction. The red/blue direction pair made moving buses read as more stops; a single blue is unambiguous against the red outbound line, and the solid-fill-inverse-of-a-stop styling carries the distinction on blue inbound lines. The stray hardcoded crowded-bar red also moved onto a proper `--red` theme token.
