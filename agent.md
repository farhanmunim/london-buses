...
│   ├── build-garage-capacity.js     # On-demand — match capacity/licences.json operating centres → garages → data/garage-capacity.json
...
│   ├── garage-capacity.json         # Per-garage operating-centre capacity (DVSA licence) — GENERATED by build-garage-capacity.js from capacity/licences.json; joined into garage-locations.json
...
| Garage filter   | `garage-filter.js`| `state.selectedGarages` (multi) → **union** of the selected garages' route-id sets |

The bus-stop and garage filters are **selection** filters: instead of chips they carry their own route-id set and re-run `applyFilters()`. The bus stop is a single selection (a pill); the garage is **multi-select** — a custom dropdown (`garage-filter.js`) of checkboxes grouped by operator (alphabetical), with the selected garages shown as removable chips. Tick several across operators and their routes are **unioned** before intersecting with the rest. In `map.js` they intersect through `_filters.stopRouteIds` / `_filters.garageRouteIds` exactly like the chip filters, so they stack with everything else (e.g. Garage = Holloway **AND** propulsion = electric). The garage filter's route-id set is supplied to `garage-filter.js` at boot via `setGarageOptions(garages, garageRoutes)` (called from `ui.js`). Clearing one — its pill ×, or the scoped Clear button (`app:garagecleared` / `app:stopcleared`) — leaves the others intact; global Clear-all (`app:resetall`) wipes all of them.
